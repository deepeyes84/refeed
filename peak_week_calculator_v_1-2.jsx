...
import { PieChart, Pie, Cell, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, LineChart, Line, CartesianGrid, RadialBarChart, RadialBar, Legend } from "recharts";

// ==========================================
// Peak Week Calculator — v1.2
// - v1.1 유지. 이 파일은 추가 기능(옵션 1,2,4,5,6,7,8,10) + 시각화 + 드래그 정렬 지원 버전
// - 모든 수식/변수 흐름은 v1.1 기반. (필요시 옵션으로 on/off)
// ==========================================
...
// ================================
// Numeric input bug fix helper (v1.1 재사용)
// ================================
function useNumericString(initial) {
  const [str, setStr] = useState(String(initial));
  const value = useMemo(() => {
    if (str === "" || str === "." || str === "-") return 0; // 입력 중
    const n = Number(str);
    return Number.isFinite(n) ? n : 0;
  }, [str]);
  const onChange = (e) => {
    // 소수점/쉼표 허용, 선행 0 자동 제거
    let s = e.target.value.replace(',', '.');
    s = s.replace(/^0+(?=\d)/, "");
    // 소수점 1개만 허용 (음수는 계산상 0으로 처리되지만 입력 중 허용)
...
// v1.2 — 컴포넌트 본체
// ==========================================
export default function PeakWeekCalculator_V12() {
  // ===== 기본 입력 =====
  ...
  const tolC = useNumericString(0.5); // g
  const tolP = useNumericString(0.5);
  const tolF = useNumericString(0.5);
  const tolKcal = useNumericString(5); // kcal

  // ===== 옵션 2: 체중/컨디션 피드백 루프 =====
  const morningWeightState = useNumericString(69.4);
  const [bloat, setBloat] = useState(0); // 0~5
  const [gi, setGi] = useState(0); // 0~5 (복부팽만/소화불편)
  const [pump, setPump] = useState(0); // 0~5
  const [applyFeedback, setApplyFeedback] = useState(false);

  // ===== 옵션 4: 세션(AM/PM) 연동 자동보정+ =====
  ...
  // ===== 옵션 5: GI 부담 관리 =====
  const fiberTarget = useNumericString(15); // g/day 기본 낮춤 (피크위크)
  const [giWeights, setGiWeights] = useState({ garaetteok: 0.2, rice: 0.15, honey: 0.5 }); // 가중치 조절 가능
  ...
  // ===== 옵션 7: 물 조절 가드레일 =====
  const minWater_ml_per_kg = useNumericString(35); // mL/kg 최저선
  ...
  // 컨디션 피드백(옵션 2) — 권고 조정값 산출
  const feedback = useMemo(() => {
    const recs = [];
    const deltaW = morningWeightState.value - weight; // 전일 대비를 모르면 절대값 경고 용도
    let carbAdjPct = 0;
    let naAdjPct = 0;
    ...
    if (bloat >= 3 || gi >= 3) {
      carbAdjPct -= 0.1; // -10%
      naAdjPct -= 0.05;  // -5%
      recs.push("복부팽만/소화불편: 탄수 -10%, Na -5% 권고");
    }
    if (pump <= 1) {
      carbAdjPct += 0.05; // +5%
      recs.push("펌프감 저하: 탄수 +5% 권고");
    }
  ...
  const carb_g_target = useMemo(() => {
    if (!applyFeedback) return carb_g_target_raw;
    return carb_g_target_raw * (1 + feedback.carbAdjPct);
  }, [applyFeedback, carb_g_target_raw, feedback.carbAdjPct]);

  // GI 부담 관리(옵션 5) — 자동튜닝
  useEffect(() => {
    if (!giAutoTune) return;
    const score = giWeights.garaetteok * (carbMix.garaetteok/100) + giWeights.rice * (carbMix.rice/100) + giWeights.honey * (carbMix.honey/100);
    // 점수가 높으면 꿀 ↓, 밥 ↑ 조정 (소폭)
    let { garaetteok, rice, honey } = carbMix;
    if (score > 0.35) { rice = clamp(rice + 5, 0, 100); honey = clamp(honey - 5, 0, 100); }
    setCarbMix({ garaetteok, rice, honey });
  }, [giAutoTune]);
  ...
  // === Auto-fix: kcal 오차를 최소화하기 위해 지방 식품 그램을 미세 조정 ===
  const kcalFromFoods =
  ...
  const kcalDiffAuto = (Pkcal + Ckcal + Fkcal) - kcalFromFoods; // target - current
  ...
  // 끼니 배분 — 세션이 있으면 peri 우선 제안
  const split = useMemo(() => {
    if (sessions.some(s => s.minutes > 0)) return MEAL_SPLITS.find(m=>m.key==="peri").values;
    return MEAL_SPLITS.find(m=>m.key===mealSplitKey)?.values || MEAL_SPLITS[0].values;
  }, [sessions, mealSplitKey]);
  ...
  const quickCarbPlan = useMemo(() => {
    ...
    const items = times.map((t, idx) => {
      ...
      const honeyCarb = gCarb * 0.7; // 위장 부담 낮춤을 위해 꿀 비중↑
  ...
  const COLORS = ["#60a5fa", "#34d399", "#f59e0b"]; // 기본 색(지정) — 시각적 구분
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50 text-slate-900">
      <div className="max-w-7xl mx-auto p-6">
        ...
        <p className="text-xs text-slate-500 mt-1">v1.1은 그대로 유지됩니다. 이 버전은 확장 기능만 추가한 별도 파일입니다.</p>
        ...
        {order.map((id)=>{
          ...
          if (id === "sources") return (
            <Section key={id} id={id} title="단백질/지방 식품 선택 & 세션 연동(옵션 4)">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
          ...
          if (id === "summary") return (
            <Section key={id} id={id} title="권장 섭취량 (하루) & 전해질">
              <div className="grid grid-cols-2 md:grid-cols-6 gap-3 text-sm">
                <Box label="단계" value={labelForPhase(phase)} />
                <Box label="탄수" value={`${round2(carb_g_target)} g`} />
                <Box label="단백질" value={`${round2(protein_g_target)} g`} />
                <Box label="지방" value={`${round2(finalTotals.F)} g`} />
                <Box label="총 열량" value={`${round2(finalKcal)} kcal`} />
                <Box label="수분(권장)" value={`${round2(water_ml/1000)} L`} warn={water_guard} />
              </div>
                ...
                <div className="bg-amber-50 rounded-xl p-3"><div className="text-amber-700">YAZIO 모드</div>
                  ...
                  {yazioMode && (
                    <div className="grid grid-cols-4 gap-2 mt-2 text-xs">
                      <Field label="tol C (g)" state={tolC} />
                      <Field label="tol P (g)" state={tolP} />
                      <Field label="tol F (g)" state={tolF} />
                      <Field label="tol kcal" state={tolKcal} step="1" />
                    </div>
                  )}
                </div>
              </div>
          ...
          if (id === "meals") return (
            <Section key={id} id={id} title="하루 4끼 식단 + 전해질">
              <TableMeals meals={meals} pSource={pSource} fSource={fSource} />
            </Section>
          );
          ...
          if (id === "diagnostics") return (
            <Section key={id} id={id} title="검산/진단 & 피드백 루프">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                <Box label="이론 kcal" value={`${round2(total_kcal_target)} kcal`} />
                <Box label="식품 합 kcal" value={`${round2(finalKcal)} kcal`} />
                <Box label="차이" value={`${round2((finalKcal-total_kcal_target))} kcal (${round2(((finalKcal-total_kcal_target)/Math.max(1,total_kcal_target))*100)}%)`} />
              </div>
              <ul className="mt-3 text-sm">
                {checks.length===0 ? <li className="px-3 py-2 bg-emerald-50 text-emerald-700 rounded-xl">✅ 오류 없음</li> : checks.map((c,i)=>(<li key={i} className={`px-3 py-2 rounded-xl ${c.level==='warn'? 'bg-amber-50 text-amber-700':'bg-slate-50 text-slate-700'}`}>• {c.msg}</li>))}
              </ul>

              <div className="mt-4 p-3 rounded-xl bg-slate-50">
                <div className="text-sm font-semibold mb-2">체중/컨디션 입력(옵션 2)</div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-2 text-sm">
                  <Field label="아침 체중(kg)" state={morningWeightState} />
                  <Select label="복부팽만" value={bloat} onChange={setBloat} options={[0,1,2,3,4,5]} />
                  <Select label="소화불편" value={gi} onChange={setGi} options={[0,1,2,3,4,5]} />
                  <Select label="펌프감" value={pump} onChange={setPump} options={[0,1,2,3,4,5]} />
                </div>
                <div className="mt-2 text-xs text-slate-600">권고: {feedback.recs.length? feedback.recs.join(' · '): '현재 특이사항 없음'}</div>
                <div className="mt-2 flex items-center gap-2"><input id="applyfb" type="checkbox" checked={applyFeedback} onChange={(e)=>setApplyFeedback(e.target.checked)} /><label htmlFor="applyfb" className="text-sm">오늘 값에 적용</label></div>
              </div>
            </Section>
          );
          ...
          if (id === "week") return (
            <Section key={id} id={id} title="D-8 ~ D-day 주간 미리보기">
              <TableWeek profile={profile} proteinLevel={proteinLevel} LBM={LBM} weight={weight} />
            </Section>
          );
          ...
          if (id === "viz") return (
            <Section key={id} id={id} title="시각화 대시보드">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card title="오늘 매크로 도넛 (kcal 기준)">
                  <ResponsiveContainer width="100%" height={240}>
                    <PieChart>
                      <Pie data={macroData} dataKey="value" nameKey="name" outerRadius={90} label>
                        {macroData.map((e,i)=> <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                      </Pie>
                      <Tooltip />
                    </PieChart>
                  </ResponsiveContainer>
                </Card>
                <Card title="D-8→D-day 매크로 타임라인 (g)">
                ...
                <Card title="글리코겐 결합수 추정 (L)">
                  <ResponsiveContainer width="100%" height={220}>
                    <LineChart data={glycogenWaterSeries}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="day" hide={false} />
                      <YAxis />
                      <Tooltip />
                      <Line type="monotone" dataKey="gw" name="결합수(L)" stroke="#60a5fa" />
                    </LineChart>
                  </ResponsiveContainer>
                </Card>
                <Card title="Na/K 게이지">
                  <div className="grid grid-cols-2">
                    <ResponsiveContainer width="100%" height={220}>
                      <RadialBarChart innerRadius="60%" outerRadius="100%" data={naGauge} startAngle={180} endAngle={0}>
                        <RadialBar minAngle={15} background clockWise dataKey="value" />
                        <Legend />
                      </RadialBarChart>
                    </ResponsiveContainer>
                    <ResponsiveContainer width="100%" height={220}>
                      <RadialBarChart innerRadius="60%" outerRadius="100%" data={kGauge} startAngle={180} endAngle={0}>
                        <RadialBar minAngle={15} background clockWise dataKey="value" />
                        <Legend />
                      </RadialBarChart>
                    </ResponsiveContainer>
            ...
            <Section key={id} id={id} title="옵션(가드레일·규칙표·보충제)">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-3 rounded-xl bg-slate-50">
                  ...
                  <Field label="최저선 mL/kg" state={minWater_ml_per_kg} step="1" />
...
function Box({ label, value, warn }) {
  return (
    <div className={`rounded-xl p-3 ${warn? 'bg-amber-50 text-amber-700':'bg-slate-50'}`}>
      <div className="text-slate-500 text-sm">{label}</div>
      <div className="font-semibold">{value}</div>
    </div>
  );
}
function Field({ label, state, step }) {
  return (
    <div>
      <label className="block text-xs mb-1">{label}</label>
      <input className="w-full border rounded px-2 py-1 text-sm" value={state.str} onChange={state.onChange} onBlur={state.onBlur} step={step||"0.1"} />
    </div>
  );
}
function Select({ label, value, onChange, options }) {
  return (
    <div>
      <label className="block text-xs mb-1">{label}</label>
      <select className="w-full border rounded px-2 py-1 text-sm" value={value} onChange={(e)=>onChange(parseInt(e.target.value))}>
...
function TableMeals({ meals, pSource, fSource }) {
  return (
...
function TableWeek({ profile, proteinLevel, LBM, weight }) {
  const rows = DAY_OPTIONS.map((opt) => {
    const ph = phaseForDay(opt.key);
    const dflt = defaultsByPhase(ph, profile);
    const gPer = dflt.carb_g_per;
    const cKind = dflt.carbKind;
    ...
    const F = Fk / 9;
    const K = Pk + Ck + Fk;
    return { day: opt.label, phase: labelForPhase(ph), P, C: carb, F, K };
  });
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
